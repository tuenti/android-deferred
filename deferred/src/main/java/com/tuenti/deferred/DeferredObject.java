 ...
 */
class DeferredObject<D, F, P> extends AbstractPromise<D, F, P> implements Deferred<D, F, P> {
	...
	@Override
	public void resolveOrReject(boolean shouldResolve, D resolve, F reject) {
		if (shouldResolve) {
			resolve(resolve);
		} else {
			reject(reject);
